#!/usr/bin/env bash
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Vision AI Scanner - Ubuntu セットアップスクリプト
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#
# 【使い方】
#   1. リポジトリをサーバーに配置
#   2. sudo bash deploy/setup-ubuntu.sh
#   3. プロンプトに従って VISION_API_KEY を入力
#   4. ブラウザで https://<サーバーIP> にアクセス
#
# 【構成】
#   nginx (HTTPS:443) → gunicorn (localhost:8000) → Flask app
...
info "=== Vision AI Scanner セットアップ開始 ==="
info "リポジトリ: ${REPO_DIR}"
...
if [[ -f "$ENV_FILE" ]]; then
    ...
    # VISION_API_KEY の入力を求める
    echo ""
    echo -e "${YELLOW}━━━ Google Cloud Vision API キーの設定 ━━━${NC}"
    echo "Vision API のキーを入力してください（後から ${ENV_FILE} で変更可能）:"
    read -r API_KEY

    if [[ -z "$API_KEY" ]]; then
        warn "APIキーが空です。後で ${ENV_FILE} に VISION_API_KEY を設定してください。"
...
if [[ -f "${SSL_DIR}/${APP_NAME}.crt" ]]; then
    ...
    openssl req -x509 -nodes -days 3650 \
        ...
        -subj "/C=JP/ST=Tokyo/O=VisionScanner/CN=${SERVER_IP}" \
...
# nginx 設定ファイルをコピー
cp "${SCRIPT_DIR}/vision-scanner.nginx" "/etc/nginx/sites-available/${APP_NAME}"
...
cp "${SCRIPT_DIR}/vision-scanner.service" "/etc/systemd/system/${APP_NAME}.service"
