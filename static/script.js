...
document.addEventListener('DOMContentLoaded', () => {
    TARGET_BOX_RATIO  = _readCssPercent('--target-box-width',  0.6);
...
/**
 * URLがhttp/httpsスキームであるか検証する（javascript: 等のXSS攻撃を防止）。
 * @param {string} url - 検証対象のURL
 * @returns {boolean} 安全なURLならtrue
 */
...
async function captureAndAnalyze() {
    ...
    try {
        ...
        // ユーザー操作によるキャンセル（モード切替/停止）: 静かに終了
        if (err.name === 'AbortError' && controller !== analyzeAbortController) {
            console.log('解析リクエストがキャンセルされました（モード切替/停止）');
            return;
        }
        // タイムアウトまたは通信失敗
        ...
        clearTimeout(timeoutId);
        if (analyzeAbortController === controller) analyzeAbortController = null;
...
function addWebResult(webDetail, data) {
    ...
    if (webDetail.pages && webDetail.pages.length > 0) {
        ...
        webDetail.pages.forEach(page => {
            ...
            // URLサニタイズ: http/httpsスキーム以外はリンクを無効化（XSS防止）
            link.href = _isSafeUrl(page.url) ? page.url : '#';
            link.target = '_blank';
...
function setMode(mode) {
    currentMode = mode;
    // モード変更時に進行中のAPIリクエストをキャンセル（レースコンディション防止）
