...
`start.sh` は初回起動時に自己署名SSL証明書を自動生成し、HTTPS モードで起動します。

**同じネットワーク内の別PCからアクセスする手順:**
...
3. 初回アクセス時、ブラウザが「この接続ではプライバシーが保護されません」と警告を表示します。
   これは自己署名証明書のためで正常です。
   - **Chrome**: 「詳細設定」→「192.168.x.x にアクセスする（安全ではありません）」をクリック
   - **Edge**: 「詳細設定」→「192.168.x.x に進む（安全ではありません）」をクリック
   - **Firefox**: 「詳細情報」→「危険性を承知で続行」をクリック

4. カメラの許可を求められたら「許可」をクリック → スキャン開始

> **証明書の再生成**: サーバーのIPアドレスが変わった場合は `certs/` フォルダを削除して `start.sh` を再実行してください。
> ```bash
> rm -rf certs/
> ./start.sh
> ```
...
| 症状 | 原因 | 対処 |
|------|------|------|
| カメラ映像が表示されない | HTTPSでないとカメラAPIがブロックされる | `localhost` で直接アクセスするか、`start.sh` でHTTPS起動する（[LAN HTTPS手順](#lan内の他のpcからカメラを使う場合https)参照） |
